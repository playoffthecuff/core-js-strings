...
function sumOfCodes(str) {
  return str ? [...str].reduce((a, v) => a + v.charCodeAt(), 0) : 0;
}
...
function formatTime(...args) {
  return args.map((v) => `${v}`.padStart(2, 0)).join`:`;
}
...
function encodeToRot13(str) {
  const getCode = (char) => {
    ...
    if (c >= a && c <= z) first = a;
    if (c >= A && c <= Z) first = A;
    return first ? ((c - first + d) % aL) + first : c;
  };
  return String.fromCharCode(...[...str].map(getCode));
}
